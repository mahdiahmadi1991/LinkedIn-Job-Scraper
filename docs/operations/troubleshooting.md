...
1. Fix the pre-check issue and rerun migration.
2. If rollback is required, prefer DB backup restore over down-migration.
...
Child tables inherit ownership from parent aggregates:
...
3. Ensure legacy single-row assumptions hold:
...
### What migration `20260306084322_AddPerUserDataOwnership` does

1. Adds nullable `AppUserId` columns.
2. Selects legacy owner as the smallest `AppUsers.Id`.
3. Backfills all existing rows to that owner.
4. Fails fast if:
...
Alternative rollback (only when safe):
...
Before down-migration, verify global uniqueness can be restored:
...
If any query returns rows, do not down-migrate. Restore from backup.
...
- 2026-03-11: Shell-safe GitHub CLI comment payloads
  - Failure pattern: using backticks inside `gh issue comment --body "..."` triggered shell command substitution and produced noisy command errors.
  - Stable fix: send multiline/comment payloads via `--body-file` (or single-quoted body) to avoid shell interpretation.
  - Guardrail: never pass Markdown text with backticks in double-quoted shell strings for `gh` commands.

- 2026-03-11: Escaped newline tokens leaked into GitHub issue text
  - Failure pattern: multiline shell text sent as `--body "...\\n..."` rendered literal `\n` in issue body/comments.
  - Stable fix: use `--body-file` (preferred) or multiline-safe quoting, and normalize escaped newline tokens before posting.
  - Guardrail: never post multiline GitHub text containing literal escaped newline tokens.

- 2026-03-11: Local system path leakage in repository docs
  - Failure pattern: absolute local workspace paths (for example `<home-dir>/...`) were written into repository markdown links, exposing local system identifiers.
  - Stable fix: replace local absolute paths with repository-relative paths and add explicit anti-leak policy in governing docs.
  - Guardrail: before push, scan for local system identifiers (absolute local paths, local usernames, workstation names, shell prompts) and sanitize them.
...
- 2026-03-10: No emergency lane for urgent main fixes
  ...
  - Stable fix: define an explicit emergency exception using `hotfix/<issue-number>-<slug>` from `main` with PR-based merge to `main`, then immediate cherry-pick sync into `develop`.
