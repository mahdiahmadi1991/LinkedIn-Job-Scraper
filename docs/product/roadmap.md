...
This plan replaces the current step-by-step delivery log in `docs/governance/plan-bridge.md` with a forward-looking portfolio-quality roadmap for evolving the repository into a strong public showcase project while preserving its current product direction:
...
### Low-friction run approach

Prefer direct documented commands for common operations:

- app run
- app + db readiness checks
- test run
- formatting or verification checks

Use helper scripts/tools only when they provide real reusable value
(for example: multi-step orchestration, validation/normalization, or safety guardrails)
that cannot be achieved cleanly with a single direct command.
...
- Mitigations:
  ...
  - keep direct local run commands explicit in docs
  - add helpers only for non-trivial multi-step flows
